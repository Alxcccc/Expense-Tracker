...
class DataBase:
    def add_expense(self, expense: Expense) -> bool:
        try:
            new_expense = expenses_db(dateExpense=expense.dateExpense, description=expense.description, amount=expense.amount)
    ...
    def summary_amount_month(self, month: int):
        try:
            with db.get_session() as session:
                result = session.query(func.sum(expenses_db.amount)).filter(extract('month', expenses_db.dateExpense) == month).scalar()
                return result
        except SQLAlchemyError as e:
            session.rollback()
            return str(e)
    ...
    def delete_expense(self, id: int):
        try:
            with db.get_session() as session:
                result = session.query(expenses_db).filter(expenses_db.idExpense == id).delete()
                if result == 0:
